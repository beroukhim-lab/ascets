...
handle_command_args <- function(args) {
  ...
  cna <<- as.data.frame(fread(arg_df$value[arg_df$flag == "-i"]))
  
  # enforce names and data classes
  names(cna) <<- c("ID", "chrom", "loc.start", "loc.end", "num.mark", "seg.mean")
  cna <<- cna %>% mutate_at(vars("loc.start", "loc.end", "seg.mean"), function(x) suppressWarnings(as.numeric(x))) %>% drop_na()
...
if(noise_supplied) {
  ...
  noisy <- noise %>% filter(noise > n) %>% drop_na()
...
cna_cyto <- cna_cyto %>%
  filter(seg.mean <= del_thresh | seg.mean >= amp_thresh) %>% # only retain segments that meet the thresholds
  mutate(alt_len = ifelse(end < loc.end & loc.start > start, (end - loc.start), # these correspond to the 4 ways two segments could overlap
         ...
         alt = ifelse(seg.mean < 0, "DEL", "AMP"), # define segments as deletions or amplifications
